...
function MenuCategory() {
    ...
    const handleCreateCategory = async () => {
        if(!unitName){
            console.error('Please Enter Unit Name')
        }
        try {
                        ...
                        {getAllunit && getAllunit.map((categoryName, index) => (
                            <TableRow key={index}>
                                <TableCell component="th" scope="row" className='table_row'>
                                    {editIndex === index ?
                                        <TextField onChange={(e) => setUnitName(e.target.value)} value={unitName.menuCategoryName} label="Category Name" variant="outlined" className="w-full col-span-3 mb-6" />
                                        :
                                ...
                                <TableCell component="th" scope="row" className='table_row'>
                                    <div className="flex w-100">
                                        ...
                                        <div className='rounded-lg bg-gray-100 p-2 ml-4 cursor-pointer table_Actions_icon2 hover:bg-red-600'>
                                            <DeleteOutlineOutlinedIcon onClick={() => handleDelete(index)} className='text-gray-600 table_icon2 ' />
                                        </div>
                                    </div>
                                </TableCell>
